...
const Navbar: React.FC = () => {
  ...
  return (
    <nav
      ...
      } fixed z-10 lg:px-14 py-4 top-0 flex w-full border-b-[0.5px] border-slate-500 bg-black/30 shadow-sm backdrop-blur`}
    >
        ...
        <div className="flex lg:items-center space-x-5 ">
          {showBackToTopButton && (
            <button
              ...
              // className={`text-white h-8 w-fit flex ml-[28%] lg:mt-0 mt-2.5 lg:ml-0 lg:items-center px-4 rounded-full border-2 border-slate-500 cursor-pointer"
            >
              <div className="border-2 mt-2 ml-[72px] lg:ml-0 lg:mt-0 rounded-full px-4 py-1.5 border-slate-500">
                Back to Top
              </div>
            </button>
          )}
          ...
          <ul
            className={`lg:flex lg:pb-0 pb-8 absolute text-center lg:static lg:z-auto z-[-1] w-full lg:w-auto transition-all ${
              navOpen
                ? "top-20 opacity-100 delay-[-100ms] duration-500 ease-in "
                : "top-[-490px] opacity-100 flex  w-full duration-[900ms] ease-out "
            }`}
          >
            <div className="flex lg:mr-8 flex-col lg:flex-row mr-[295px] lg:space-x-12 ">
